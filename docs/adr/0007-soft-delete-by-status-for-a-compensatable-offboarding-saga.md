...
- Scope: the portal's account-deletion saga — microservice-offboarding (orchestrator),
  microservice-memes and microservice-comments (participants)
...
**A status on the aggregate, not a separate entity.** `MemeMetadata` and `Comment` carry
...
per service, as a database view (`active_memes`, `active_comments`): every read goes through it,
so a query cannot forget the condition — there is no condition to forget — and a build-time guard
(`MemeReadFilterTest`, `CommentReadFilterTest`) fails the build if any SQL outside the one
erasure-aware adapter names the base table. That guard is a source-level rule rather than ArchUnit
on purpose: ArchUnit reasons about types, and this rule is about the text inside a string constant.
...
(`memes_erasure_backlog`, `comments_erasure_backlog`) and says in the log that the content is
hidden but NOT erased and that nothing will delete it on a timer. A lost closure is a GDPR problem;
...
- **Not yet converted: microservice-user-collections.** It still erases the leaver's favourites on
  the mark, so a compensated saga restores memes and comments but not the saved list. The change is
  the same shape as the two here (status column, filtering view, three commands) and is the
  outstanding follow-up; until it lands, "the saga is fully compensatable" is true of content
  people can read and not of one private list.
